...
class PyBaMMSimulator:
    ...
    def run_protocol(
        self,
        c_rates: Optional[List[float]],
        step_durations: Optional[List[float]] = None,
        verbose: bool = False,
        charge_steps: Optional[List[str]] = None,
    ) -> Dict:
        """
        ...
        Args:
            ...
            charge_steps: Optional PyBaMM step strings for the charging portion of
                the experiment. When provided, these override c_rates and
                step_durations while keeping the same discharge/relaxation prefix.
        ...
        if not charge_steps:
            if c_rates is None:
                raise ValueError("c_rates must be provided when charge_steps is not used")
    ...
    def _build_experiment(
        self,
        c_rates: Optional[List[float]],
        step_durations: Optional[List[float]],
        charge_steps: Optional[List[str]] = None,
    ) -> pybamm.Experiment:
        """
        ...
        Args:
            ...
            step_durations: List of durations [minutes] for each step
            charge_steps: Optional explicit PyBaMM step strings for the charging
                stage. When provided, c_rates and step_durations are ignored.
    ...
    def _extract_charge_throughput(self, solution) -> float:
        ...
        integration_fn = getattr(np, "trapezoid", None)
        if integration_fn is None:
            integration_fn = np.trapz
        Q_integrated = integration_fn(-current[charging_mask], time_sim[charging_mask])
    ...
    def _extract_sei_metrics(self, solution) -> Dict[str, float]:
        ...
        try:
            ...
            if sei_thickness is None:
                raise KeyError(f"No SEI thickness variable found in {possible_names}")
