...
DEFAULT_RUN_ID = "stage-one-run-v1"
FIXED_TAIL_MINUTES = 3.0
PUBLIC_ANCHOR_ALIASES = {
    "baseline": BASELINE_HEAD,
    "seed": "UA3_TRBO_3.00C_rest10m_4.15C",
}
FOLLOWUP_ANCHOR_PROTOCOLS = {
    "UA3_TRBO_3.00C_rest10m_4.15C": {
        "name": "UA3_TRBO_3.00C_rest10m_4.15C",
        "description": "Fixed three-step seed protocol used to start the public stage-one search.",
        "type": "OursStageOneAnchor",
        "c_rates": [3.0, 0.0, 4.15],
        "step_durations": [10.0, 10.0, 10.0],
        "parameters": {
            "source_note": "Fixed public release seed",
        },
    },
}
SEARCH_DIMENSIONS = ("first_rate", "rest_minutes", "third_rate", "tail_rate")
SEARCH_BOUNDS = {
    "first_rate": (2.9, 3.1),
...
def public_protocol_label(protocol_name: str | None) -> str | None:
    ...
    if protocol_name == BASELINE_HEAD:
        return "baseline"
    if protocol_name == PUBLIC_ANCHOR_ALIASES["seed"]:
        return "seed"
...
def protocol_signature(protocol: Dict[str, Any]) -> tuple[tuple[float, ...], tuple[float, ...]]:
    return (
...
def default_tail_rate(third_rate: float) -> float:
    return round_search_value("tail_rate", third_rate - 1.25)


def protocol_point(protocol: Dict[str, Any]) -> tuple[float, ...]:
    third_rate = round_search_value("third_rate", float(protocol["c_rates"][-1]))
    tail_rate = (
        round_search_value("tail_rate", float(protocol["c_rates"][-1]))
        if len(protocol["c_rates"]) >= 4
        else default_tail_rate(third_rate)
    )
    return (
...
def build_tapered_candidate(
    anchor_protocol: Dict[str, Any],
    point: tuple[float, ...],
    source: str,
    iteration: int,
) -> Dict[str, Any]:
    first_rate, rest_minutes, third_rate, tail_rate = point
    active_minutes = round((30.0 - rest_minutes) / 2.0, 2)
    taper_entry_minutes = round(active_minutes - FIXED_TAIL_MINUTES, 2)
    return {
        "name": (
            f"UA4_VTBO_{first_rate:.2f}C_rest{format_rest_label(rest_minutes)}m_"
            f"{third_rate:.2f}C_tail{tail_rate:.2f}C"
        ),
        "description": (
            f"Voltage-aware tapered trust-region BO candidate around {anchor_protocol['name']} "
            f"with {first_rate:.2f}C / rest {rest_minutes:.0f}m / "
            f"{third_rate:.2f}C entry / {tail_rate:.2f}C tail"
        ),
        "type": "UA4_VTBO_Candidate",
        "c_rates": [round(first_rate, 4), 0.0, round(third_rate, 4), round(tail_rate, 4)],
        "step_durations": [
            active_minutes,
...
def make_point(offset_point: tuple[float, ...]) -> tuple[float, ...]:
    return (
        ...
        round_search_value("tail_rate", offset_point[3]),
    )
...
def evaluate_protocol(protocol: Dict[str, Any], scenarios: Sequence[Scenario], simulator_cls: Any, verbose: bool = False) -> List[Dict[str, Any]]:
    ...
    for scenario in scenarios:
        ...
        metrics = simulator.run_and_extract(protocol["c_rates"], protocol["step_durations"], verbose=False)
...
def sample_candidate_pool(
    ...
    for _ in range(pool_size * 6):
        ...
        for index, dimension in enumerate(SEARCH_DIMENSIONS):
            ...
            elif dimension == "tail_rate":
                raw_value = rng.triangular(local_lower, local_upper, local_lower)
            else:
    ...
    *,
    ...
    while len(adaptive_protocols) < budget:
        ...
        if next_point in {item["point"] for item in observations}:
            ...
            break
        protocol = build_tapered_candidate(anchor_protocol, next_point, source=source, iteration=iteration)
...
def run_search(
    *,
    ...
    output_dir = quest_root / "experiments" / "runs" / "stage_one" / "results"
