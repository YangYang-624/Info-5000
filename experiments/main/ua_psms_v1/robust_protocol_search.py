...
DEFAULT_RUN_ID = "run-psms-ua4-v1"
FIXED_TAIL_MINUTES = 3.0
EVENT_TRIGGER_GUARD_MINUTES = 2.0
MIN_MONITOR_MINUTES = 1.0
TAIL_RATE_DROP = 1.25
HOLD_TO_TAIL_DROP = 0.55
DEFAULT_TRIGGER_VOLTAGE = 4.16
DEFAULT_HOLD_MINUTES = 0.0
HOLD_STAGE_MASK_THRESHOLD = 0.5
TAIL_VOLTAGE_LIMIT = 4.20
FOLLOWUP_ANCHOR_PROTOCOLS = {
    "UA3_3.00C_rest10m_3.80C": {
        "name": "UA3_3.00C_rest10m_3.80C",
        "description": "Legacy robust protocol kept for backward-compatible replay of the earlier follow-up line.",
        "type": "UA3_TRBO_Anchor",
        "c_rates": [3.0, 0.0, 3.8],
        "step_durations": [10.0, 10.0, 10.0],
        "parameters": {
            "source_run_id": "run-ua3-local-v1",
            "source_summary": "experiments/main/ua_cmobo_v1/results/ua3_local_search_summary.json",
        },
    },
    "UA3_TRBO_3.00C_rest10m_3.95C": {
        "name": "UA3_TRBO_3.00C_rest10m_3.95C",
        "description": "Current strongest safe protocol used as the frontier-followup anchor.",
        "type": "UA3_TRBO_Anchor",
        "c_rates": [3.0, 0.0, 3.95],
        "step_durations": [10.0, 10.0, 10.0],
        "parameters": {
            "source_run_id": "run-fa-trbo-ua3-v1",
            "source_summary": "experiments/main/ua_trbo_v1/results/ua_fa_trbo_search_summary.json",
        },
    },
    "UA3_TRBO_3.00C_rest10m_4.15C": {
        "name": "UA3_TRBO_3.00C_rest10m_4.15C",
        "description": "Current strongest safe protocol used as the event-triggered voltage-boundary anchor.",
        "type": "UA4_EVTBO_Anchor",
        "c_rates": [3.0, 0.0, 4.15],
        "step_durations": [10.0, 10.0, 10.0],
        "parameters": {
            "source_run_id": "run-fa-frontier-ua3-v1",
            "source_summary": "experiments/main/ua_trbo_v1/results/ua_fa_frontier_search_summary.json",
        },
    },
    "UA4_VTBO_3.00C_rest10m_4.25C_tail2.40C": {
        "name": "UA4_VTBO_3.00C_rest10m_4.25C_tail2.40C",
        "description": "Current measured incumbent from the fixed-tail voltage-aware proxy line.",
        "type": "UA4_EVTBO_Anchor",
        "c_rates": [3.0, 0.0, 4.25, 2.40],
        "step_durations": [10.0, 10.0, 7.0, 3.0],
        "parameters": {
            "source_run_id": "run-vtbo-ua4-v1",
            "source_summary": "experiments/main/ua_vtbo_v1/results/ua_vtbo_search_summary.json",
            "tail_rate": 2.40,
            "tail_minutes": 3.0,
        },
    },
    "UA4_EVTBO_3.00C_rest10m_4.30C_v4p16": {
        "name": "UA4_EVTBO_3.00C_rest10m_4.30C_v4p16",
        "description": "Current strongest event-triggered incumbent used as the anchor for the masked multi-stage scaffold.",
        "type": "UA5_PSMS_Anchor",
        "c_rates": [3.0, 0.0, 4.30, 4.30, 3.05],
        "step_durations": [10.0, 10.0, 2.0, 5.0, 3.0],
        "charge_steps": [
            "Charge at 3.00 C for 10.00 minutes",
            "Rest for 10.00 minutes",
            "Charge at 4.30 C for 2.00 minutes",
            "Charge at 4.30 C for 5.00 minutes or until 4.16 V",
            "Charge at 3.05 C for 3.00 minutes or until 4.20 V",
        ],
        "parameters": {
            "source_run_id": "run-evtbo-ua4-v1",
            "source_summary": "experiments/main/ua_evtbo_v1/results/ua_evtbo_search_summary.json",
            "trigger_voltage": 4.16,
            "guard_minutes": 2.0,
            "event_monitor_minutes": 5.0,
            "hold_rate": None,
            "hold_minutes": 0.0,
            "hold_enabled": False,
            "tail_rate": 3.05,
...
SEARCH_BOUNDS = {
    ...
    "entry_rate": (4.05, 4.50),
    "trigger_voltage": (4.14, 4.19),
    "hold_rate": (2.20, 3.60),
    "hold_minutes": (0.0, 2.0),
}
SEARCH_ROUNDING = {
    "first_rate": 0.05,
    "rest_minutes": 1.0,
...
TRUST_REGION_SEED_OFFSETS = (
    (0.0, 0.0, 0.05, 0.10, 0.00, -0.25, 0.0),
    (0.0, -1.0, 0.10, 0.15, -0.01, -0.15, 0.5),
    (0.0, 1.0, 0.05, 0.10, 0.01, -0.10, 1.0),
    (0.05, 0.0, 0.05, 0.15, 0.00, -0.20, 0.5),
    (-0.05, 0.0, 0.05, 0.10, -0.02, -0.30, 0.0),
    (0.0, 0.0, 0.10, 0.20, 0.02, -0.25, 1.5),
)
SEQUENCE_FEASIBILITY_PRIOR_FLOOR = 0.20
...
class AblationConfig:
    ...
    disable_entry_activation: bool = False
    disable_stage_mask: bool = False

    def is_active(self) -> bool:
        return any(
            (
                self.disable_sequence_prior,
                self.disable_entry_activation,
                self.disable_stage_mask,
            )
        )

    def suffix(self) -> str:
        parts: List[str] = []
        if self.disable_sequence_prior:
            parts.append("noprior")
        if self.disable_entry_activation:
            parts.append("noentry")
        if self.disable_stage_mask:
            parts.append("nomask")
        return "_".join(parts)
    ...
    def label(self) -> str:
        parts: List[str] = []
        if self.disable_sequence_prior:
            parts.append("no-sequence-prior")
        if self.disable_entry_activation:
            parts.append("no-entry-activation")
        if self.disable_stage_mask:
            parts.append("no-stage-mask")
        return ", ".join(parts) if parts else "full-psms"

    def to_dict(self) -> Dict[str, Any]:
        return {
            "active": self.is_active(),
            "disable_sequence_prior": self.disable_sequence_prior,
            "disable_entry_activation": self.disable_entry_activation,
            "disable_stage_mask": self.disable_stage_mask,
            "label": self.label(),
            "suffix": self.suffix(),
        }
...
def find_quest_root(start: Path) -> Path:
    for parent in (start, *start.parents):
        if (parent / "quest.yaml").exists() and (parent / "baselines").exists():
            return parent
        # Allow the cleaned project/ export to serve as a runnable root even
        # though it no longer carries DeepScientist quest metadata files.
        if (
...
def ensure_baseline_import_path(quest_root: Path) -> Path:
    baseline_root = quest_root / "baselines" / "local" / "ax-pybamm-fastcharge"
...
def default_hold_rate(third_rate: float) -> float:
    lower, _ = SEARCH_BOUNDS["hold_rate"]
    upper = max(lower, third_rate - 0.10)
    proposed = third_rate - HOLD_TO_TAIL_DROP
    return round_search_value("hold_rate", clamp(proposed, lower, upper))


def default_entry_rate(third_rate: float) -> float:
    return round_search_value("entry_rate", third_rate)


def default_hold_minutes() -> float:
    return round_search_value("hold_minutes", DEFAULT_HOLD_MINUTES)


def format_hold_minutes_label(hold_minutes: float) -> str:
    rounded = round_search_value("hold_minutes", hold_minutes)
    if float(rounded).is_integer():
        return f"{int(rounded)}"
    return f"{rounded:.1f}".replace(".", "p")


def enforce_hold_rate(third_rate: float, hold_rate: float) -> float:
    lower, _ = SEARCH_BOUNDS["hold_rate"]
    upper = max(lower, third_rate - 0.10)
    return round_search_value("hold_rate", clamp(hold_rate, lower, upper))


def enforce_entry_rate(third_rate: float, entry_rate: float) -> float:
    lower, upper = SEARCH_BOUNDS["entry_rate"]
    return round_search_value("entry_rate", clamp(entry_rate, max(lower, third_rate), upper))


def derive_tail_rate(third_rate: float, hold_rate: float, hold_enabled: bool) -> float:
    base_rate = hold_rate if hold_enabled else third_rate
    lower, upper = 1.5, 3.5
    return round(clamp(base_rate - HOLD_TO_TAIL_DROP, lower, min(upper, base_rate - 0.10)), 2)
...
def canonicalize_point(
    ...
    ablation = ablation_config or AblationConfig()
    first_rate = round_search_value("first_rate", float(point[0]))
    rest_minutes = round_search_value("rest_minutes", float(point[1]))
    third_rate = round_search_value("third_rate", float(point[2]))
    entry_source = third_rate if ablation.disable_entry_activation else float(point[3])
    entry_rate = enforce_entry_rate(third_rate, entry_source)
    trigger_voltage = round_search_value("trigger_voltage", float(point[4]))
    if ablation.disable_stage_mask:
        hold_rate = default_hold_rate(third_rate)
        hold_minutes = round_search_value("hold_minutes", 0.0)
    else:
        hold_rate = enforce_hold_rate(third_rate, float(point[5]))
        hold_minutes = round_search_value("hold_minutes", float(point[6]))
    return (
        first_rate,
        rest_minutes,
        third_rate,
...
def protocol_point(
    ...
    entry_rate = enforce_entry_rate(
        third_rate,
        float(parameters.get("entry_rate", default_entry_rate(third_rate))),
    )
    trigger_voltage = round_search_value(
        "trigger_voltage",
        float(parameters.get("trigger_voltage", default_trigger_voltage(third_rate))),
    )
    hold_rate_value = parameters.get("hold_rate")
    if hold_rate_value is None:
        hold_rate_value = default_hold_rate(third_rate)
    hold_rate = round_search_value(
        "hold_rate",
        float(hold_rate_value),
    )
    hold_minutes_value = parameters.get("hold_minutes")
    if hold_minutes_value is None:
        hold_minutes_value = default_hold_minutes()
    hold_minutes = round_search_value(
        "hold_minutes",
        float(hold_minutes_value),
    )
...
def build_event_trigger_candidate(
    ...
    (
        ...
        hold_rate_raw,
        hold_minutes_raw,
    ) = canonicalize_point(point, ablation)
    active_minutes = round((30.0 - rest_minutes) / 2.0, 2)
    entry_rate = enforce_entry_rate(third_rate, entry_rate_raw)
    hold_minutes = 0.0 if ablation.disable_stage_mask else round_search_value("hold_minutes", hold_minutes_raw)
    hold_enabled = (not ablation.disable_stage_mask) and (hold_minutes >= HOLD_STAGE_MASK_THRESHOLD)
    if not hold_enabled:
        hold_minutes = 0.0
    hold_rate = default_hold_rate(third_rate) if ablation.disable_stage_mask else enforce_hold_rate(third_rate, hold_rate_raw)
    monitor_minutes = round(
        max(active_minutes - EVENT_TRIGGER_GUARD_MINUTES - FIXED_TAIL_MINUTES - hold_minutes, MIN_MONITOR_MINUTES),
        2,
    )
    available_hold_minutes = round(active_minutes - EVENT_TRIGGER_GUARD_MINUTES - FIXED_TAIL_MINUTES - monitor_minutes, 2)
    if hold_enabled:
        hold_minutes = round_search_value("hold_minutes", max(0.0, available_hold_minutes))
        hold_enabled = hold_minutes >= HOLD_STAGE_MASK_THRESHOLD
    if not hold_enabled:
        hold_minutes = 0.0
    tail_rate = derive_tail_rate(third_rate, hold_rate, hold_enabled)
    charge_steps = [
        f"Charge at {first_rate:.2f} C for {active_minutes:.2f} minutes",
        f"Rest for {rest_minutes:.2f} minutes",
        f"Charge at {entry_rate:.2f} C for {EVENT_TRIGGER_GUARD_MINUTES:.2f} minutes",
        (
    ...
    if hold_enabled:
        charge_steps.append(
            f"Charge at {hold_rate:.2f} C for {hold_minutes:.2f} minutes or until {TAIL_VOLTAGE_LIMIT - 0.01:.2f} V"
        )
        c_rates.append(round(hold_rate, 4))
        step_durations.append(hold_minutes)
    charge_steps.append(
        f"Charge at {tail_rate:.2f} C for {FIXED_TAIL_MINUTES:.2f} minutes or until {TAIL_VOLTAGE_LIMIT:.2f} V"
    )
    c_rates.append(round(tail_rate, 4))
    step_durations.append(FIXED_TAIL_MINUTES)
    base_name = (
        f"UA5_PSMS_{first_rate:.2f}C_rest{format_rest_label(rest_minutes)}m_"
        f"{third_rate:.2f}C_e{entry_rate:.2f}_v{format_voltage_label(trigger_voltage)}_"
        f"h{hold_rate:.2f}_{format_hold_minutes_label(hold_minutes)}m"
    )
    protocol_name = f"{base_name}_{ablation.suffix()}" if ablation.is_active() else base_name
    description_prefix = (
        f"Ablation ({ablation.label()}) of the physics-prior masked multi-stage candidate"
        if ablation.is_active()
        else "Physics-prior masked multi-stage candidate"
    )
    return {
        "name": protocol_name,
        "description": (
            f"{description_prefix} around {anchor_protocol['name']} "
            f"with {first_rate:.2f}C / rest {rest_minutes:.0f}m / "
            f"{entry_rate:.2f}C entry + {EVENT_TRIGGER_GUARD_MINUTES:.0f}m guard / "
            f"{third_rate:.2f}C monitor until trigger {trigger_voltage:.2f}V / "
            f"{hold_rate:.2f}C hold for {hold_minutes:.1f}m / "
            f"{tail_rate:.2f}C tail"
        ),
        "type": "UA5_PSMS_Ablation_Candidate" if ablation.is_active() else "UA5_PSMS_Candidate",
        "c_rates": c_rates,
        ...
        "parameters": {
            ...
            "entry_boost": round(entry_rate - third_rate, 4),
            ...
            "stage_count": len(c_rates),
            "physics_prior_entry_monotone": entry_rate >= third_rate,
            "physics_prior_monotone_tail": tail_rate < (hold_rate if hold_enabled else third_rate),
...
def make_point(
    ...
    return canonicalize_point(
        (
            ...
            round_search_value("entry_rate", offset_point[3]),
            round_search_value("trigger_voltage", offset_point[4]),
            round_search_value("hold_rate", offset_point[5]),
            round_search_value("hold_minutes", offset_point[6]),
        ),
        ablation_config=ablation_config,
    )


def build_initial_trust_region_points(
    anchor_protocol: Dict[str, Any],
    budget: int,
    ablation_config: AblationConfig | None = None,
) -> List[tuple[float, ...]]:
    if budget <= 0:
        return []
    max_seed_points = min(budget, len(TRUST_REGION_SEED_OFFSETS) + 1)
    anchor_point = protocol_point(anchor_protocol, ablation_config)
    ...
    for (
        ...
        hold_rate_offset,
        hold_minutes_offset,
    ) in TRUST_REGION_SEED_OFFSETS:
        candidate = make_point(
            (
                anchor_point[0] + first_offset,
                anchor_point[1] + rest_offset,
                anchor_point[2] + third_offset,
                anchor_point[3] + entry_offset,
                anchor_point[4] + trigger_offset,
                anchor_point[5] + hold_rate_offset,
                anchor_point[6] + hold_minutes_offset,
            ),
            ablation_config,
        )
...
def fit_gp_model(observations: Sequence[Dict[str, Any]]) -> GaussianProcessRegressor | None:
    ...
    kernel = (
        ...
        * Matern(length_scale=0.25, length_scale_bounds=(1e-2, 5.0), nu=2.5)
...
def sample_candidate_pool(
    ...
    for _ in range(pool_size * 8):
        ...
        point = canonicalize_point(tuple(point_values), ablation)
        if point in seen_points or point in pool_seen:
            continue
        pool.append(point)
        pool_seen.add(point)
    ...
    for index, dimension in enumerate(SEARCH_DIMENSIONS):
        ...
        for candidate in candidates:
            point = canonicalize_point(tuple(candidate), ablation)
            if point not in seen_points and point not in pool_seen:
                pool.append(point)
                pool_seen.add(point)
    return pool
...
def sequence_feasibility_prior(
    protocol: Dict[str, Any],
    ablation_config: AblationConfig | None = None,
) -> float:
    ablation = ablation_config or AblationConfig()
    if ablation.disable_sequence_prior:
        return 1.0
    parameters = protocol.get("parameters", {})
    entry_rate = float(parameters.get("entry_rate", parameters.get("third_rate", 0.0)))
    third_rate = float(parameters.get("third_rate", entry_rate))
    trigger_margin = float(parameters.get("trigger_margin_voltage", 0.0))
    hold_enabled = bool(parameters.get("hold_enabled", False))
    hold_minutes = float(parameters.get("hold_minutes", 0.0))
    hold_rate = float(parameters.get("hold_rate") or third_rate)
    tail_rate = float(parameters.get("tail_rate") or hold_rate)

    score = 1.0
    entry_boost = max(0.0, entry_rate - third_rate)
    if entry_boost > 0.0:
        score -= 0.35 * min(1.0, entry_boost / 0.25)
        if not hold_enabled:
            score -= 0.15 * min(1.0, entry_boost / 0.20)

    if trigger_margin < 0.04:
        score -= 0.25 * min(1.0, (0.04 - trigger_margin) / 0.04)

    if hold_enabled and hold_minutes > 0.0:
        hold_drop = max(0.0, third_rate - hold_rate)
        score += 0.10 * min(1.0, hold_drop / 0.30)
    elif not ablation.disable_stage_mask:
        score -= 0.10

    if tail_rate >= third_rate - 0.05:
        score -= 0.10

    return max(SEQUENCE_FEASIBILITY_PRIOR_FLOOR, clip_unit_interval(score))
...
def propose_next_point(
    ...
    candidate_pool = sample_candidate_pool(
        state.center,
        state.radii,
        seen_points,
        seed=seed,
    ...
    for point, mean, sigma, guard_mean, guard_sigma in zip(
        ...
        protocol = build_event_trigger_candidate(
            anchor_protocol,
            point,
            source="acquisition_prior",
            iteration=-1,
            ablation_config=ablation_config,
        )
        sequence_prior = sequence_feasibility_prior(protocol, ablation_config=ablation_config)
        ...
        acquisition = sequence_prior * (
            (safe_probability * performance_bonus) + (feasibility_weight * safe_probability)
        )
...
def run_trust_region_controller(
    *,
    ...
    anchor_point = protocol_point(anchor_protocol, ablation)
    state = TrustRegionState(
        center=anchor_point,
    ...
    seed_points = build_initial_trust_region_points(anchor_protocol, budget, ablation)
    ...
    return {
        ...
        "controller_summary": {
            ...
            "ablation": ablation.to_dict(),
            "sequence_feasibility_prior_floor": SEQUENCE_FEASIBILITY_PRIOR_FLOOR,
            "feasibility_floor": feasibility_floor,
            "feasibility_weight": feasibility_weight,
...
def run_search(
    *,
    ...
    feasibility_floor: float,
    feasibility_weight: float,
    disable_sequence_prior: bool,
    disable_entry_activation: bool,
    disable_stage_mask: bool,
    verbose: bool,
) -> Dict[str, Any]:
    ...
    ablation = AblationConfig(
        disable_sequence_prior=disable_sequence_prior,
        disable_entry_activation=disable_entry_activation,
        disable_stage_mask=disable_stage_mask,
    )
