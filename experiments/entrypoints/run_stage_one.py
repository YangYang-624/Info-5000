#!/usr/bin/env python3
"""User-facing entrypoint for the stage-one search pipeline."""
...
from stage_one import DEFAULT_RUN_ID, run_search


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the public stage-one search pipeline."
    )
    parser.add_argument("--mode", choices=("smoke", "search"), default="smoke")
    parser.add_argument("--anchor-name", type=str, default="seed")
...
def main() -> None:
    ...
    output_prefix = args.output_prefix or (
        "stage_one_smoke" if args.mode == "smoke" else "stage_one_main"
    )
    run_id = args.run_id or (
        "stage-one-smoke-v1" if args.mode == "smoke" else DEFAULT_RUN_ID
    )
    summary = run_search(
        mode=args.mode,
