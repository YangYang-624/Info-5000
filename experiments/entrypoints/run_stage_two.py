#!/usr/bin/env python3
"""User-facing entrypoint for the stage-two search pipeline."""
...
from stage_two import DEFAULT_RUN_ID, run_search


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the public stage-two search pipeline."
    )
    parser.add_argument("--mode", choices=("smoke", "search"), default="smoke")
    parser.add_argument("--anchor-name", type=str, default="stage_one")
...
def main() -> None:
    ...
    output_prefix = args.output_prefix or (
        "stage_two_smoke" if args.mode == "smoke" else "stage_two_main"
    )
    run_id = args.run_id or (
        "stage-two-smoke-v1" if args.mode == "smoke" else DEFAULT_RUN_ID
    )
    summary = run_search(
        mode=args.mode,
